...
.get(cors.cors, (req, res, next) => {
    Comments.find(req.query) //req.query will contain the query we want to find the comments with. i.e. req.query will contain => dish: 'some dishid' 
    .populate('author')
    .then((comments) => {
        res.statusCode = 200;
        ...
        if(comment != null){
            if(comment.author.equals(req.user._id)){
                ...
                .then((comment) => {
                    ...
                    Commentss.findById(comment._id)
                    .populate('author')
                    .then((comment) => {
                        res.statusCode = 200;
                        res.setHeader('Content-Type', 'application/json');
